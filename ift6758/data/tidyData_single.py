...
def tidyData_single(dfs: pd.DataFrame, last_idx: int):
    """
    ...
    #if last event already processed return empty dataframe
    if len(dfs.iloc[:, 0]["liveData"]["plays"]["allPlays"]) == last_idx+1:
        return pd.DataFrame([]),last_idx
    #get all plays up to idx
    allPlays = dfs.iloc[:, 0]["liveData"]["plays"]["allPlays"][last_idx+1:]
    last_idx = len(dfs.iloc[:, 0]["liveData"]["plays"]["allPlays"])-1
    ...
    if last_idx >= 0:
        last_event_cache = dfs.iloc[:, 0]["liveData"]["plays"]["allPlays"][last_idx]
    ...
    for event in allPlays:
        ...
        if event['result']['eventTypeId'] == "SHOT":
            ...
            game_id.append(dfs.iloc[:, 0].name)
            strength.append('NA')
            awayTeam.append(dfs.iloc[:, 0]['gameData']['teams']['away']['name'])
            homeTeam.append(dfs.iloc[:, 0]['gameData']['teams']['home']['name'])
            #check for rinkSide info and append
            try:
                homeSide.append(dfs.iloc[:, 0]['liveData']['linescore']['periods'][0]['home']['rinkSide'])
            except:
                homeSide.append('NA')
